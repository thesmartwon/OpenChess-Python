...
Value Stockfish::PieceValue[PHASE_NB][PIECE_NB] = {
...
bool Position::legal(Move m, Bitboard pinned) const {
  ...
  if(! (color_of(moved_piece(m)) == us))
      return false;
  if(! (piece_on(king_square(us)) == make_piece(us, KING)))
      return false;
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (st->checkersBB)
  {
      if (type_of(pc) != KING)
      {
          ...
          if (more_than_one(st->checkersBB))
              return false;

          // Our move must be a blocking evasion or a capture of the checking piece
          if (!((between_bb(lsb(st->checkersBB), king_square(us)) | st->checkersBB) & to))
              return false;
      }
