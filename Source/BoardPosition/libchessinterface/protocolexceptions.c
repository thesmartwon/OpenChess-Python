...
void protocolexceptions_SetByBinName(struct chessinterfaceengine* e,
const char* binname) {
    // if this is Ildtiadar, skip UCI detection
    // since we prefer CECPv2.
    if (strstr(binname, "Ildtiadar") || strstr(binname, "ildtiadar")) {
        e->skipuci = 1;
    }    
}
